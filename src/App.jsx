...
/***********************************
 * Artan Protec — Red/White/Black (Light + Dark toggle)
 * - Safe BrowserRouter for Netlify (remember /public/_redirects)
 * - Desktop: Navbar dropdowns (Products, Industries) with icons
 * - Mobile: Compact slide-down menu with nested submenus
 * - Hero background video (webm/mp4 + poster)
 * - All previous content wired in (Home, Products x3, Industries index + 6 pages,
 *   About, Downloads, Contact/Netlify form)
 * - Product photography hooks via /public/images/*.jpg (placeholders provided)
 ***********************************/
...
function Navbar() {
  ...
  return (
    <header className={cx("fixed top-0 inset-x-0 z-50 border-b", isLight ? "bg-white/80 border-slate-200 backdrop-blur" : "bg-black/80 border-white/10 backdrop-blur")}>      
      <Container className="flex h-16 items-center justify-between">
        {/* Brand */}
        <Link to="/" className="flex items-center gap-3">
          <img src="/artan-protec-logo.png" alt="Artan Protec" className="h-8 w-auto" onError={(e)=>e.currentTarget.style.display='none'} />
          <div className="leading-tight">
            <p className={cx("font-semibold", isLight ? "text-slate-900" : "text-white")}>Artan Protec</p>
            <p className={cx("text-[11px]", isLight ? "text-slate-500" : "text-slate-400")}>Advanced Protection | Engineered Performance</p>
          </div>
        </Link>
        ...
        <nav className="hidden md:flex items-center gap-6 text-sm">
          ...
          <MenuGroup label={<span className={cx(isLight?"text-slate-700":"text-slate-200")}>Products</span>} active={isActive("/products")}
            onEnter={()=>setOpenProd(true)} onLeave={()=>setOpenProd(false)}>
            {openProd && <Drop items={productItems} onClick={()=>setOpenProd(false)} />}
          </MenuGroup>

          <MenuGroup label={<span className={cx(isLight?"text-slate-700":"text-slate-200")}>Industries</span>} active={isActive("/industries")}
            onEnter={()=>setOpenInd(true)} onLeave={()=>setOpenInd(false)}>
            {openInd && <Drop items={industryItems} onClick={()=>setOpenInd(false)} />}
          </MenuGroup>

          <Link to="/downloads" className={linkBase}>
            <span className={cx(isActive("/downloads") && "font-bold text-red-600", isLight ? "text-slate-700" : "text-slate-200")}>Downloads</span>
          ...
          <Link to="/contact" className={linkBase}>
            <span className={cx(isActive("/contact") && "font-bold text-red-600", isLight ? "text-slate-700" : "text-slate-200")}>Contact</span>
        ...
        <div className="flex items-center gap-3">
          ...
          <button aria-label="Toggle theme" onClick={toggle}
            className={cx("rounded-lg px-3 py-2 text-xs border", isLight?"border-slate-300 text-slate-700 hover:bg-slate-50":"border-white/15 text-white hover:bg-white/10")}
          >
            {isLight?"Light":"Dark"}
          </button>
          {/* Mobile menu button */}
          <button className={cx("md:hidden rounded-lg p-2 border", isLight?"border-slate-300 text-slate-700":"border-white/15 text-white")} onClick={()=>setMobileOpen((v)=>!v)}>
          ...
          <Container className="py-3 space-y-2">
            ...
            <details className={cx("px-3 py-2 rounded-lg", isLight?"bg-white":"bg-white/5")}>
              ...
              <div className="mt-2 ml-3 space-y-2">
                {productItems.map((it)=> (
                  <Link key={it.to} to={it.to} onClick={()=>setMobileOpen(false)} className={cx("block px-3 py-2 rounded-lg", isLight?"hover:bg-slate-50":"hover:bg-white/10")}>{it.title}</Link>
                ))}
              </div>
            </details>
            <details className={cx("px-3 py-2 rounded-lg", isLight?"bg-white":"bg-white/5")}>
...
function Home() {
  ...
  const cases = [
    ...
    { title: "Industrial Gloves", bullets: ["Para-aramid blends for cut/heat resistance","Cone‑wound CoreThread for consistent stitch","Programmatic sizing & private labeling"] },
    { title: "Thermal Barriers", bullets: ["Targeted GSM fabrics for insulation","Liner options for comfort & moisture","Batch traceability for audits"] },
  ];
    ...
    <div className={cx(isLight?"bg-gradient-to-b from-white via-slate-50 to-red-50/20 text-slate-900":"bg-slate-950 text-white")}>      
      ...
      <section className="relative overflow-hidden">
        <div className="absolute inset-0 -z-10">
          ...
          <div className={cx("absolute inset-0", isLight?"bg-gradient-to-b from-white/70 via-white/70 to-white/85":"bg-[radial-gradient(1200px_600px_at_20%_-10%,rgba(239,68,68,0.18),transparent)]")}/>
        </div>
        <Container className="pt-24 pb-24">
          <div className="max-w-3xl">
            <div className={cx("inline-flex items-center gap-2 rounded-full px-3 py-1 text-[11px] uppercase tracking-[0.2em]", isLight?"border border-red-200 bg-red-50/60 text-red-700":"border border-red-500/30 bg-red-900/20 text-red-200")}>Aramid Materials • FRR Fabrics • Industrial PPE</div>
            <h1 className="mt-4 text-4xl sm:text-6xl font-extrabold leading-tight">Advanced Protection. Engineered Performance.</h1>
            <p className={cx("mt-5 text-lg", isLight?"text-slate-700":"text-slate-300")}>High‑strength, heat‑resistant yarns & threads (CoreThread), FR fabrics (ArmorWeave), and select PPE assemblies (ArmorShield) for mission‑critical environments.</p>
            <div className="mt-8 flex flex-wrap items-center gap-3">
              ...
              <div className="relative group">
                ...
                <div className="absolute hidden group-hover:block bg-white text-black mt-2 rounded-xl shadow-xl ring-1 ring-slate-200 w-72 p-2 z-10">
                  <Link to="/products/corethread" className="flex items-center gap-3 rounded-lg px-3 py-2 hover:bg-slate-50"><Icon.Thread className="h-4 w-4"/> CoreThread</Link>
                  <Link to="/products/armorweave" className="flex items-center gap-3 rounded-lg px-3 py-2 hover:bg-slate-50"><Icon.Fabric className="h-4 w-4"/> ArmorWeave</Link>
                  <Link to="/products/armorshield" className="flex items-center gap-3 rounded-lg px-3 py-2 hover:bg-slate-50"><Icon.Shield className="h-4 w-4"/> ArmorShield</Link>
                </div>
              </div>
              <Button href="/downloads" variant="outline"><Icon.Download className="h-4 w-4"/> Tech Sheets</Button>
            </div>
      ...
      <Section id="about" eyebrow="About" title="Artan Protec" lead="A specialized brand focused on aramid inputs and FR protective solutions — built for reliability, documentation, and scale.">
        <div className="grid gap-6 md:grid-cols-2">
          <Card>
            <p className={cx("text-sm", isLight?"text-slate-600":"text-slate-200")}>We convert technical fibers into performance components: yarns & threads for high‑temperature seams, FR fabrics for shells/liners/barriers, and selected PPE assemblies to accelerate pilots. Our process emphasizes application fit, quality control, and consistent supply.</p>
            <ul className={cx("mt-4 grid grid-cols-2 gap-3 text-sm", isLight?"text-slate-700":"text-slate-200")}> 
              ...
              <li className="flex items-center gap-2">✔ Shade & logo customization</li>
          ...
          <Card>
            ...
            <div className="grid grid-cols-2 gap-3">
              <picture>
                <img src="/images/product-corethread.jpg" alt="CoreThread sample" className="h-28 w-full object-cover rounded-lg ring-1 ring-slate-200" onError={(e)=>e.currentTarget.style.display='none'} />
              </picture>
              <picture>
                <img src="/images/product-armorweave.jpg" alt="ArmorWeave sample" className="h-28 w-full object-cover rounded-lg ring-1 ring-slate-200" onError={(e)=>e.currentTarget.style.display='none'} />
              </picture>
              <picture>
                <img src="/images/product-armorshield.jpg" alt="ArmorShield sample" className="h-28 w-full object-cover rounded-lg ring-1 ring-slate-200" onError={(e)=>e.currentTarget.style.display='none'} />
              </picture>
              <picture>
                <img src="/images/product-lab.jpg" alt="Lab / testing" className="h-28 w-full object-cover rounded-lg ring-1 ring-slate-200" onError={(e)=>e.currentTarget.style.display='none'} />
              </picture>
            </div>
            <p className={cx("mt-2 text-xs", isLight?"text-slate-500":"text-slate-400")}>Drop your own images into /public/images/ with the same names to replace these placeholders.</p>
          </Card>
        </div>
      </Section>

      {/* PRODUCT LINES OVERVIEW */}
      <Section id="catalog" eyebrow="Products" title="Product Lines" lead="Open a product line to view detailed specs, variants, and downloads.">
        <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
          <Card className="hover:shadow-[0_20px_60px_-20px_rgba(239,68,68,0.25)] transition">
            <div className="flex items-start gap-4">
              <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-red-600 to-black text-white ring-1 ring-red-300/40"><Icon.Thread className="h-5 w-5"/></span>
              <div>
                <h3 className={cx("text-lg font-semibold", isLight?"text-slate-900":"text-white")}>CoreThread — Aramid Yarns & Threads</h3>
                <p className={cx("text-sm mt-1", isLight?"text-slate-600":"text-slate-300")}>Industrial aramid yarns & sewing threads engineered for heat/FR, strength and durability in PPE seams, hot‑end filtration and thermal assemblies.</p>
              </div>
            </div>
            <div className="mt-4"><Link to="/products/corethread" className="text-red-700 hover:underline">Learn more →</Link></div>
          </Card>
          <Card className="hover:shadow-[0_20px_60px_-20px_rgba(239,68,68,0.25)] transition">
            <div className="flex items-start gap-4">
              <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-red-600 to-black text-white ring-1 ring-red-300/40"><Icon.Fabric className="h-5 w-5"/></span>
              <div>
                <h3 className={cx("text-lg font-semibold", isLight?"text-slate-900":"text-white")}>ArmorWeave — FRR & PPE Fabrics</h3>
            ...
            <div className="mt-4"><Link to="/products/armorweave" className="text-red-700 hover:underline">Learn more →</Link></div>
          </Card>
          <Card className="hover:shadow-[0_20px_60px_-20px_rgba(239,68,68,0.25)] transition">
            <div className="flex items-start gap-4">
              <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-red-600 to-black text-white ring-1 ring-red-300/40"><Icon.Shield className="h-5 w-5"/></span>
              <div>
                <h3 className={cx("text-lg font-semibold", isLight?"text-slate-900":"text-white")}>ArmorShield — Industrial PPE & Components</h3>
            ...
            <div className="mt-4"><Link to="/products/armorshield" className="text-red-700 hover:underline">Learn more →</Link></div>
          </Card>
        </div>
      </Section>
      ...
      <Section id="why" eyebrow="Why Artan" title="What sets us apart">
        <div className="grid gap-6 md:grid-cols-3">
          <Card><div className="flex items-start gap-4"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-red-600 to-black text-white ring-1 ring-red-300/40"><Icon.Thread className="h-5 w-5"/></span><div><h4 className={cx("font-semibold", isLight?"text-slate-900":"text-white")}>Materials mastery</h4><p className={cx("text-sm mt-1", isLight?"text-slate-600":"text-slate-300")}>Meta‑ & para‑aramid systems tuned from fiber to seam.</p></div></div></Card>
          <Card><div className="flex items-start gap-4"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-red-600 to-black text-white ring-1 ring-red-300/40"><Icon.Shield className="h-5 w-5"/></span><div><h4 className={cx("font-semibold", isLight?"text-slate-900":"text-white")}>Quality & compliance</h4><p className={cx("text-sm mt-1", isLight?"text-slate-600":"text-slate-300")}>Documentation, batch traceability, and process control.</p></div></div></Card>
          <Card><div className="flex items-start gap-4"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-red-600 to-black text-white ring-1 ring-red-300/40"><Icon.Fabric className="h-5 w-5"/></span><div><h4 className={cx("font-semibold", isLight?"text-slate-900":"text-white")}>Program flexibility</h4><p className={cx("text-sm mt-1", isLight?"text-slate-600":"text-slate-300")}>Shade/logo customization and scalable partner network.</p></div></div></Card>
        </div>
      </Section>
      ...
      <Section id="industries" eyebrow="Industries" title="Where we fit" lead="Solutions aligned to sector requirements and safety expectations.">
        <div className="grid gap-4 md:grid-cols-6 text-sm">
          {["PPE","Filtration","Telecom","Utilities","Oil & Gas","Fire Services"].map((name) => (
            <Link key={name} to={`/industries/${name.toLowerCase().replace(/ & /g,'').replace(/\s+/g,'')}`} className={cx("text-center rounded-xl px-4 py-3 ring-1", isLight?"ring-slate-200 bg-white hover:bg-slate-50 text-slate-700":"ring-white/10 bg-white/5 hover:bg-white/10 text-slate-200")}>{name}</Link>
          ))}
        </div>
      </Section>
      ...
      <Section id="standards" eyebrow="Standards" title="Certifications & standards">
        <Card>
          <div className={cx("grid grid-cols-2 md:grid-cols-4 gap-4 text-sm", isLight?"text-slate-700":"text-slate-300")}> 
            ...
            <div className="rounded-lg bg-white px-3 py-2 text-center ring-1 ring-slate-200">ASTM / NFPA (app‑specific)</div>
          ...
          <div className="flex items-start justify-between gap-6">
            <div>
              ...
              <ul className={cx("mt-2 list-disc pl-5 text-sm", isLight?"text-slate-700":"text-slate-200")}>
                {cases[caseIdx].bullets.map((b) => (<li key={b}>{b}</li>))}
              </ul>
            </div>
            <div className="flex items-center gap-2">
              {cases.map((_, i) => (
                <button key={i} onClick={()=>setCaseIdx(i)} aria-label={`Show case ${i+1}`} className={cx("h-2.5 w-2.5 rounded-full", i===caseIdx?"bg-red-600":"bg-red-200")} />
              ))}
            </div>
          </div>
        </Card>
        ...
        <Card>
          <div className="grid md:grid-cols-2 gap-6 items-center">
            <svg viewBox="0 0 320 260" className="w-full h-auto">
              <defs>
                <linearGradient id="g1" x1="0" x2="1"><stop offset="0%" stopColor="#fecaca" stopOpacity="0.55"/><stop offset="100%" stopColor="#ef4444" stopOpacity="0.55"/></linearGradient>
                <linearGradient id="g2" x1="0" x2="1"><stop offset="0%" stopColor="#e5e7eb" stopOpacity="0.55"/><stop offset="100%" stopColor="#9ca3af" stopOpacity="0.55"/></linearGradient>
                <linearGradient id="g3" x1="0" x2="1"><stop offset="0%" stopColor="#fde68a" stopOpacity="0.6"/><stop offset="100%" stopColor="#f87171" stopOpacity="0.6"/></linearGradient>
              </defs>
              <rect x="10" y="220" width="300" height="20" fill="#ffffff" stroke="#e2e8f0" />
              <g className="group"><rect x="30" y="180" width="260" height="24" fill="url(#g1)" stroke="#ef4444" className="transition duration-200 group-hover:opacity-90" /><text x="40" y="196" fill="#7f1d1d" fontSize="10">ArmorWeave: Outer shell (ripstop/twill)</text></g>
              <g className="group"><rect x="40" y="150" width="240" height="22" fill="url(#g2)" stroke="#6b7280" className="transition duration-200 group-hover:opacity-90" /><text x="50" y="165" fill="#374151" fontSize="10">Liner: comfort & moisture options</text></g>
              <g className="group"><rect x="50" y="120" width="220" height="20" fill="url(#g3)" stroke="#f59e0b" className="transition duration-200 group-hover:opacity-90" /><text x="60" y="134" fill="#92400e" fontSize="10">Barrier: thermal/arc reinforcement</text></g>
              <g className="group"><rect x="60" y="90" width="200" height="18" fill="#111827" opacity="0.2" stroke="#0f172a" /><text x="70" y="103" fill="#111827" fontSize="10">Seams: CoreThread aramid sewing</text></g>
            </svg>
            <div className={cx("space-y-3 text-sm", isLight?"text-slate-700":"text-slate-200")}> 
              <div className="flex items-start gap-3"><div className="h-3 w-3 rounded-sm" style={{background:"linear-gradient(90deg,#fecaca88,#ef444488)"}} /><div><p className={cx("font-medium", isLight?"text-slate-900":"text-white")}>ArmorWeave outer shell</p><p className={cx(isLight?"text-slate-500":"text-slate-400")}>Ripstop/twill, shade options, abrasion resistance.</p></div></div>
              <div className="flex items-start gap-3"><div className="h-3 w-3 rounded-sm" style={{background:"linear-gradient(90deg,#e5e7eb88,#9ca3af88)"}} /><div><p className={cx("font-medium", isLight?"text-slate-900":"text-white")}>Comfort liner</p><p className={cx(isLight?"text-slate-500":"text-slate-400")}>Moisture/comfort finishes for extended wear.</p></div></div>
              <div className="flex items-start gap-3"><div className="h-3 w-3 rounded-sm" style={{background:"linear-gradient(90deg,#fde68a90,#f8717190)"}} /><div><p className={cx("font-medium", isLight?"text-slate-900":"text-white")}>Thermal/arc barrier</p><p className={cx(isLight?"text-slate-500":"text-slate-400")}>Higher GSM or blends to meet protection targets.</p></div></div>
              <div className="flex items-start gap-3"><div className="h-3 w-3 rounded-sm bg-slate-800/40" /><div><p className={cx("font-medium", isLight?"text-slate-900":"text-white")}>CoreThread seams</p><p className={cx(isLight?"text-slate-500":"text-slate-400")}>Aramid threads and constructions for seam integrity.</p></div></div>
            </div>
          </div>
        </Card>
      </Section>
      ...
      <Section id="get-started" eyebrow="Get started" title="Spec a material or request a pilot">
        <div className="flex flex-wrap gap-3">
          <Button href="/downloads" variant="outline"><Icon.Download className="h-4 w-4"/> Download brochures</Button>
          <Button href="/contact"><Icon.Mail className="h-4 w-4"/> Contact sales</Button>
        </div>
      </Section>
    </div>
  );
...
const VariantTable = ({ rows }) => (
  <div className="overflow-x-auto"><table className={tableCls}><thead className="bg-slate-50 text-slate-600"><tr className="border-b border-slate-200"><Th>Variant</Th><Th>Spec</Th><Th>Suited For</Th></tr></thead><tbody>{rows.map((r)=>(<tr key={r.name} className="border-b border-slate-200 last:border-0"><Td><span className="font-medium text-slate-900">{r.name}</span></Td><Td className="text-slate-700">{r.spec}</Td><Td className="text-slate-700">{r.suited}</Td></tr>))}</tbody></table></div>
);
      ...
      <Section eyebrow="Products" title={fam.name} lead={fam.summary}>
        <div className="grid gap-6 md:grid-cols-3">
          <Card className="md:col-span-2">
            ...
            <ul className={cx("list-disc pl-5 space-y-1 text-sm", isLight?"text-slate-700":"text-slate-200")}>{fam.details.map((d)=>(<li key={d}>{d}</li>))}</ul>
...
const FAMILIES = [
  { id:"corethread", path:"/products/corethread", name:"CoreThread — Aramid Yarns & Threads", summary:"Industrial aramid yarns & sewing threads engineered for heat/FR, strength and durability in PPE seams, hot‑end filtration and thermal assemblies.", details:["Constructions: 2–8 ply staple‑based; tex/dtex/denier per application (e.g., 750 ±25 denier 4‑ply).","Performance: breaking strength ≥60 N (spec‑dependent); elongation 3.0–4.4%; high modulus.","Chemistry: meta‑aramid & para‑aramid blends; dope‑dyed meta‑aramid colors available.","Finish: sewing lubricant, anti‑wick, soft‑hand; cone winding & doubling in‑house."], variants:[{name:"CT‑750/4P", spec:"750 ±25 denier, 4‑ply", suited:"PPE seams, filtration sewing" },{ name:"CT‑1000/3P", spec:"~1000 denier class, 3‑ply", suited:"Heavy protective gear, gloves" },{ name:"CT‑MetaDyed", spec:"Dope‑dyed meta‑aramid colors", suited:"Brand‑matched FR apparel" }], downloads:[{ label:"CoreThread Datasheet (PDF)", href:"/brochures/artan-corethread.pdf" }] },
  { id:"armorweave", path:"/products/armorweave", name:"ArmorWeave — FRR & PPE Fabrics", summary:"FR fabrics for coveralls, jackets, gloves, hoods and thermal liners. Targeted GSM and weaves for shell, liner and barrier layers.", details:["Fiber systems: meta/para‑aramid; optional antistatic grids; ripstop/twill constructions.","Weights: typical 150–280 GSM for apparel; higher GSM for barrier layers.","Finishes: comfort & moisture options; shade ranges aligned with industry norms.","Compliance: application‑specific testing pathways and documentation guidance."], variants:[{ name:"AW‑Shell180", spec:"180 GSM ripstop shell", suited:"FR coveralls & jackets" },{ name:"AW‑Liner150", spec:"150 GSM liner", suited:"Comfort liner for PPE" },{ name:"AW‑Barrier260", spec:"260 GSM barrier", suited:"Thermal barrier / added protection" }], downloads:[{ label:"ArmorWeave Catalog (PDF)", href:"/brochures/artan-armorweave.pdf" }] },
  { id:"armorshield", path:"/products/armorshield", name:"ArmorShield — Industrial PPE & Components", summary:"Select PPE gear and components leveraging our aramid inputs to accelerate pilots and early adoption.", details:["Cut/heat resistant gloves & sleeves; FR hoods and jackets with aramid seams.","Partnered manufacturing with QA; traceability & batch documentation available.","Customization: logos, shade matching, size grading per program needs."], variants:[{ name:"AS‑GlovePro", spec:"Cut/heat‑resistant glove", suited:"Foundry, welding, hot‑process handling" },{ name:"AS‑HoodFR", spec:"FR hood with aramid seams", suited:"Fire safety & industrial" },{ name:"AS‑JacketAR", spec:"Arc‑rated jacket (program)", suited:"Utilities & electrical maintenance" }], downloads:[{ label:"PPE Program Overview (PDF)", href:"/brochures/artan-ppe.pdf" }] },
];

/************** Non-product pages **************/
function ProductsIndex(){
  const famByPath = FAMILIES;
  return (
    <main>
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
          {famByPath.map((f)=>(
            <Card key={f.id}>
              ...
              <div className="mt-4"><Link to={f.path} className="text-red-700 hover:underline">Open →</Link></div>
            </Card>
          ))}
        </div>
      </Section>
...
function IndustriesIndex(){
  const items = [
    ...
    { slug:"telecom", name:"Telecom", copy:"Thermal barriers & safety gear" },
    { slug:"utilities", name:"Utilities", copy:"Arc‑rated programs" },
    { slug:"oilgas", name:"Oil & Gas", copy:"FR apparel & barriers" },
  ...
  return (
    <main>
      <Section eyebrow="Downloads" title="Brochures & spec sheets">
        <div className="flex flex-wrap gap-3">
          <Button href="/brochures/artan-corethread.pdf" variant="outline" newTab><Icon.Download className="h-4 w-4"/> CoreThread</Button>
          <Button href="/brochures/artan-armorweave.pdf" variant="outline" newTab><Icon.Download className="h-4 w-4"/> ArmorWeave</Button>
          <Button href="/brochures/artan-ppe.pdf" variant="outline" newTab><Icon.Download className="h-4 w-4"/> PPE Program</Button>
        </div>
      </Section>
    </main>
  );
  ...
  return (
    <main>
      <Section eyebrow="About" title="About Artan Protec" lead="Focused on engineered FR performance from fiber to finished gear.">
        <div className="grid gap-6 md:grid-cols-2">
          <Card>
            <p className="text-sm text-slate-700">Artan Protec delivers high-performance aramid yarns, threads, fabrics and PPE components. Our teams align material properties with application realities to enable reliable, documented, and scalable programs.</p>
          </Card>
          <Card>
            <h4 className="font-semibold mb-2">At a glance</h4>
            <div className="grid grid-cols-2 gap-4 text-sm text-slate-700">
              <div><p className="text-slate-500">Product families</p><p className="font-semibold">3</p></div>
              <div><p className="text-slate-500">Target GSM range</p><p className="font-semibold">150–280 (apparel)</p></div>
              <div><p className="text-slate-500">Thread constructions</p><p className="font-semibold">2–8 ply staple</p></div>
              <div><p className="text-slate-500">Customization</p><p className="font-semibold">Logos & shades</p></div>
            </div>
          </Card>
        </div>
      </Section>
  ...
  return (
    <main>
      <Section eyebrow="Contact" title="Talk to our team" lead="Submit specs or drawings — we'll respond with the right thread, fabric or PPE program.">
        <div className="grid gap-6 md:grid-cols-2">
          <Card>
            <div className="flex items-center gap-2 font-semibold"><Icon.Mail className="h-5 w-5"/> Emails</div>
            <ul className="mt-2 text-slate-700 text-sm space-y-2">
              <li><strong>Primary:</strong> <a className="hover:underline" href="mailto:artanprotec@gmail.com">artanprotec@gmail.com</a></li>
            </ul>
          ...
          <Card>
            <div className="flex items-center gap-2 font-semibold"><Icon.Phone className="h-5 w-5"/> Phone</div>
        ...
        <Card className="mt-6">
          <form name="contact" method="POST" data-netlify="true" netlify-honeypot="bot-field" className="grid md:grid-cols-3 gap-4">
            ...
            <div className="md:col-span-3"><Button type="submit"><Icon.Mail className="h-4 w-4"/> Submit</Button></div>
          </form>
        </Card>
      </Section>
    </main>
